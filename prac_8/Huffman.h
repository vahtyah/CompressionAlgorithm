...
bool first(int,std::string&);
bool first(std::string);
std::string retCodeData(std::string);
std::string givenEnCode(char);
std::string givenDeCode(int x, int bit);
		...
		if (tree[currNode].left == -1 && tree[currNode].right == -1) {
			if (currNode == NYT) {
				...
				if (p < 10) {
					code += data[a++];
					p = b_to_d(code);
					output += givenDeCode(p, 5);
				}
				else {
					output += givenDeCode(p, 4);
				}
			}
			else {
...
bool first(int n, std::string &data) {
	for (int a = 0; a < n; a++) {
		if (data[a] == data[n]) {
			return false;
		}
...
std::string givenEnCode(char str) {
	switch (str) {
